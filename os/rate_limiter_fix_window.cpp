...
struct ms_clock {
    ...
    static time_point now() noexcept {
        return time_point(chrono::duration_cast<duration>(chrono::steady_clock::now().time_since_epoch()));
    }
};
...
class RateLimiter {
    ...
    public:
        ...
        m_limit(limit), m_interval(interval), m_interval_count(interval.count()) {}
        ...
        bool operator()() const;
        double qps() const {
            return 1000.0 * this->m_limit / this->m_interval_count;
        }
